...
function lp_get_version_code() { return "0.1"; }

/**
* player specified size.
*/
function lp_get_player_modal() { return 740; }
function lp_get_player_width() { return lp_get_player_modal() - 30; }
function lp_get_player_height() { return lp_get_player_width() * 9 / 19; }

// get the default vhost for players.
function lp_get_player_vhost() { return "players"; }
// the api server port, for chat room.
function lp_get_api_server_port() { return 8085; }
// the lp http server port
function lp_get_lp_http_server_port() { return 8080; }
...
function lp_get_player_publish_vhost(src_vhost) { return (src_vhost != lp_get_player_vhost())? src_vhost:(src_vhost + "_pub"); }
// for chat, use rtmp only vhost, low latecy, without gop cache.
function lp_get_player_chat_vhost(src_vhost) { return (src_vhost != lp_get_player_vhost())? src_vhost:(src_vhost + "_chat"); }
...
*/
function update_nav() {
    $("#lp_index").attr("href", "index.html" + window.location.search);
    $("#nav_lp_player").attr("href", "lp_player.html" + window.location.search);
    $("#nav_lp_publisher").attr("href", "lp_publisher.html" + window.location.search);
    $("#nav_lp_chat").attr("href", "lp_chat.html" + window.location.search);
    $("#nav_lp_bwt").attr("href", "lp_bwt.html" + window.location.search);
...
function build_default_publish_rtmp_url() {
    ...
    if (server == vhost || vhost == "") {
        ...
        vhost = lp_get_player_chat_vhost(vhost);
        return "rtmp://" + server + ":" + port + "/" + app + "...vhost..." + vhost + "/" + stream;
    }
}
...
*/
function lp_init_rtmp(rtmp_url, modal_player) {
    lp_init(rtmp_url, null, modal_player);
}
function lp_init_hls(hls_url, modal_player) {
    lp_init(null, hls_url, modal_player);
}
function lp_init(rtmp_url, hls_url, modal_player) {
    update_nav();
    ...
    if (modal_player) {
        $(modal_player).width(lp_get_player_modal() + "px");
        $(modal_player).css("margin-left", "-" + lp_get_player_modal() / 2 +"px");
    }
}
// for the chat to init the publish url.
function lp_init_publish(rtmp_url) {
    update_nav();
...
function lp_init_bwt(rtmp_url, hls_url) {
    update_nav();
...
function lp_initialize_codec_page(
    cameras, microphones,
...
*/
function lp_publisher_initialize_page(
    cameras, microphones,
    sl_cameras, sl_microphones, sl_vcodec, sl_profile, sl_level, sl_gop, sl_size, sl_fps, sl_bitrate,
    sl_acodec
) {
    lp_initialize_codec_page(
        cameras, microphones,
        sl_cameras, sl_microphones, sl_vcodec, sl_profile, sl_level, sl_gop, sl_size, sl_fps, sl_bitrate,
        sl_acodec
    );
...
*/
function lp_chat_initialize_page(
    cameras, microphones,
    sl_cameras, sl_microphones, sl_vcodec, sl_profile, sl_level, sl_gop, sl_size, sl_fps, sl_bitrate,
    sl_acodec
) {
...
*/
function lp_publiser_get_codec(
    vcodec, acodec,
    sl_cameras, sl_microphones, sl_vcodec, sl_profile, sl_level, sl_gop, sl_size, sl_fps, sl_bitrate,
    sl_acodec
) {
