...
*/
function LpPlayer(container, width, height, private_object) {
    if (!LpPlayer.__id) {
        LpPlayer.__id = 100;
    }
    if (!LpPlayer.__players) {
        LpPlayer.__players = [];
    }
    
    LpPlayer.__players.push(this);
    
    this.private_object = private_object;
    this.container = container;
    this.width = width;
    this.height = height;
    this.id = LpPlayer.__id++;
    this.stream_url = null;
    this.buffer_time = 0.3; // default to 0.3
    this.volume = 1.0; // default to 100%
    this.callbackObj = null;
    this.Lp_player_url = "lp_player/release/lp_player.swf?_version="+lp_get_version_code();
...
* callbacks:
*      on_player_ready():int, when lp player ready, user can play.
*      on_player_metadata(metadata:Object):int, when lp player get metadata.
*/
LpPlayer.prototype.start = function(url) {
    if (url) {
        this.stream_url = url;
    }
    ...
    flashvars.on_player_ready = "__lp_on_player_ready";
    flashvars.on_player_metadata = "__lp_on_player_metadata";
    flashvars.on_player_timer = "__lp_on_player_timer";
    flashvars.on_player_empty = "__lp_on_player_empty";
    flashvars.on_player_full = "__lp_on_player_full";
