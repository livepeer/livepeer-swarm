...
// 	Transcode(in io.Stream, out io.Stream)
// }
...
// rtmpMux, err := joy4rtmp.Dial("rtmp://localhost:" + et.localTranscoderRTMPPort + "/stream/" + string(src.StreamID))
...
func (et *ExternalTranscoder) LocalSRSUploadMux() (av.MuxCloser, error) {
	...
	if err != nil {
		glog.Errorf("Transcoder RTMP Stream Publish Error: %v", err)
		return nil, err
	}
	return rtmpMux, nil
}
...
func (et *ExternalTranscoder) StartUpload(ctx context.Context, rtmpMux av.MuxCloser, src *lpmsio.Stream) error {
...
func (et *ExternalTranscoder) StartDownload(ctx context.Context, hlsMux *lpmsio.Stream) error {
...
type HLSDownloader interface {
	Download(pc chan *m3u8.MediaPlaylist, sc chan *lpmsio.HLSSegment) error
}
...
func (d SRSHLSDownloader) Download(pc chan *m3u8.MediaPlaylist, sc chan *lpmsio.HLSSegment) error {
	before := time.Now()
	...
	for {
		...
		for _, seginfo := range pl.Segments {
			...
			if _, found := d.cache.Get(seginfo.URI); found == false {
				...
				sc <- &lpmsio.HLSSegment{Name: seginfo.URI, Data: seg}
...
// func (et *ExternalTranscoder) Transcode(demux av.DemuxCloser, mux io.Stream) error {
// 	dstConn, err := joy4rtmp.Dial("rtmp://localhost:" + et.localTranscoderRTMPPort + "/stream/" + string(mux.StreamID))
// 	if err != nil {
// 		return ErrTranscoderConnRefused
// 	}

// 	upErrC := make(chan error, 1)
// 	go func() { upErrC <- avutil.CopyFile(dstConn, demux) }()
